...
function Hexo(base, args) {
  console.log(chalk.red(__filename),chalk.yellow(">> New Hexo Objects"));
...
Hexo.prototype.init = function() {
  console.log(chalk.red(__filename),chalk.yellow(">> Init Hexo Objects"));
  var self = this;
  ...
  // Load internal plugins in ../plugins
...
Hexo.prototype.load = function(callback) {
  console.log(chalk.red(__filename),chalk.yellow(">> BEGIN Load() in Hexo"));
  var self = this;
  return loadDatabase(this).then(function() {
    self.log.info('Start processing - load');
    return Promise.all([
      self.source.process(), //<<= Load content of file to database
      self.theme.process()
    ])
  }).then(function() {
    console.log(chalk.red(__filename),chalk.yellow(">> END Load() in Hexo"));
...
Hexo.prototype.watch = function(callback) {
  ...
  return loadDatabase(this).then(function() {
    self.log.info('Start processing - watch');
...
Hexo.prototype._generate = function(options) {
  console.log(chalk.red(__filename),chalk.yellow(">> _generate in Hexo"));
  ...
  var self = this;
  var routeList = route.list(); // << Nothing here
  var log = this.log;
  ...
  function Locals(path, locals) {
    console.log(chalk.red(__filename),chalk.yellow(">> New Local Objects"),chalk.red(path));
...
Hexo.prototype.exit = function(err) {
  console.log(chalk.red(__filename),chalk.yellow(">> Exit in Hexo local"));
  var self = this;
