...
module.exports = function(ctx) {
  console.log(chalk.red(__filename),chalk.yellow(">> Load Plugin"));
...
function loadModules(ctx) {
  ...
  return loadModuleList(ctx).map(function(name) {
    ...
    // return ctx.loadPlugin(path,function() {
    //   ctx.log.debug('Plugin loaded: %s', chalk.magenta(name));
    //   console.log('New-Plugin loaded: %s', chalk.magenta(name))
    // },function(err) {
    //   ctx.log.error({err: err}, 'Plugin load failed: %s', chalk.magenta(name));
    //   console.log({err: err}, 'Plugin load failed: %s', chalk.magenta(name))
    // })
