...
function Box(ctx, base, options) {
  console.log(chalk.red(__filename),chalk.yellow(">> New Box Objects"));
...
Box.prototype._createFileClass = function() {
  ...
  _File.prototype.render = function(options, callback) {
    console.log(chalk.red(__filename),chalk.yellow(">> Render Box Objects"),chalk.green(options));
...
Box.prototype.process = function(callback) {
  console.log(chalk.red(__filename),chalk.yellow(">> Process Box Objects"));
  var self = this;
  var base = this.base;
  var Cache = this.Cache;
  var ctx = this.context; 
  return fs.stat(base).then(function(stats) {
    ...
    return self._readDir(base, function(file) {
      ...
      return Promise.filter(cacheFiles, function(path) {
        return !~files.indexOf(path); // If path not in files filter will return True and this path will be delete
      }).map(function(path) {
...
Box.prototype.watch = function(callback) {
  console.log(chalk.red(__filename),chalk.yellow(">> Watch Box Objects"));
  if (this.isWatching()) {
